...
const Vehicles = () => {
    ...
    const idvehicles = location.pathname.split("/")[3];
    const navigate = useNavigate();

    useEffect(() => {
        const fetchAllVehicles = async () => {
            if (!idvehicles || isNaN(idvehicles)) {
                setErrors({ fetch: "Invalid vehicle ID" });
                return;
            }
            setLoading(true);
            try{
                const res = await axios.get(`http://localhost:8800/vehicles/${idvehicles}`);
                console.log("Fetched vehicle data:", res.data); 
                ...
                setErrors({ fetch: err.response?.data?.error || "Failed to fetch vehicle" });
    ...
    const handleClick = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.put(`http://localhost:8800/vehicles/${idvehicles}`, vehicle);
            navigate("/vehicles");
        } catch (err) {
            setErrors(err.response?.data?.error || "Failed to update vehicle");
            console.error("AxiosError:", err);
        } finally {
            setLoading(false);
        }
