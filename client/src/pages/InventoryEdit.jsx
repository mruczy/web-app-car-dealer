...
const InventoryEdit = () => {
    ...
    const idinventory = location.pathname.split("/")[3];
    const navigate = useNavigate();

    useEffect(() => {
        const fetchData = async () => {
            if (!idinventory || isNaN(idinventory)) {
                setErrors({ fetch: "Invalid inventory ID" });
                return;
            }
            setLoading(true);
            try {
                const [inventoryRes, vehiclesRes] = await Promise.all([
                    axios.get(`http://localhost:8800/inventory/${idinventory}`),
                    axios.get("http://localhost:8800/vehicles"),
                ]);
                console.log("Fetched inventory data:", inventoryRes.data); // Debug log
    ...
    const handleClick = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.put(`http://localhost:8800/inventory/${idinventory}`, {
            ...
            setErrors({ submit: err.response?.data?.error || "Failed to update inventory item" });
            console.error("AxiosError:", err);
        } finally {
            setLoading(false);
        }
